; Liberty Oxygen Orion GUI Wallet Installer for Windows
; Copyright (c) 2017-2020, The Liberty Project
; See LICENSE
#define GuiVersion GetFileVersion("bin\liberty-wallet-gui.exe")

[Setup]
AppName=Liberty GUI Wallet
...
DefaultDirName={commonpf}\Liberty GUI Wallet
DefaultGroupName=Liberty GUI Wallet
UninstallDisplayIcon={app}\liberty-wallet-gui.exe
...
LicenseFile=LICENSE
AppPublisher=The Liberty Developer Community
AppPublisherURL=https://getliberty.org
TimeStampsInUTC=yes
...
; Background info:
; This installer indeed asks for admin rights so the Liberty files can be copied to a place where they have at least
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Liberty.

Source: {#file AddBackslash(SourcePath) + "ReadMe.htm"}; DestDir: "{app}"; DestName: "ReadMe.htm"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy
Source: "LICENSE"; DestDir: "{app}"; Flags: ignoreversion

; Liberty GUI wallet exe and guide
Source: "bin\liberty-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\LibertyChainGUI-wallet-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; Liberty CLI wallet
Source: "bin\extras\liberty-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\liberty-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; Liberty wallet RPC interface implementation
Source: "bin\extras\liberty-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Liberty daemon
Source: "bin\libertyd.exe"; DestDir: "{app}"; Flags: ignoreversion

; Liberty daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "liberty-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Liberty blockchain utilities
Source: "bin\extras\liberty-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\liberty-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\liberty-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\liberty-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\liberty-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\liberty-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\liberty-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\liberty-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\liberty-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\liberty-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\liberty-gen-ssl-cert.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\liberty-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitliberty');
  s := 'The default folder to store the Liberty blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\liberty-project\liberty-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" Liberty default location
...
begin
  Result := BlockChainDir('') + '\bitliberty.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "liberty-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Liberty daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\libertyd.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\liberty-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Liberty GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\liberty-wallet-gui.exe";
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\LibertyChainGUI-wallet-guide.pdf"; IconFilename: "{app}\liberty-wallet-gui.exe"
...
Name: "{group}\Utilities\Liberty Daemon"; Filename: "{app}\libertyd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\liberty-wallet-cli.exe"; WorkingDir: "{userdocs}\Liberty\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\Liberty\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\liberty-wallet-gui\liberty-wallet-gui.log"""
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\liberty-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM libertyd.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\liberty-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\liberty-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\liberty-project\liberty-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\liberty-project\liberty-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "liberty:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "liberty://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "liberty"; ValueType: "string"; ValueData: "URL:Liberty Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "liberty"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "liberty\DefaultIcon"; ValueType: "string"; ValueData: "{app}\liberty-wallet-gui.exe,0"
Root: HKCR; Subkey: "liberty\shell\open\command"; ValueType: "string"; ValueData: """{app}\liberty-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "libertyseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "libertyseed://a.b.c.d:port"
Root: HKCR; Subkey: "libertyseed"; ValueType: "string"; ValueData: "URL:Liberty Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "libertyseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "libertyseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\liberty-wallet-gui.exe,0"
Root: HKCR; Subkey: "libertyseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\liberty-wallet-gui.exe"" ""%1"""
