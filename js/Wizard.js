...
function getApproximateBlockchainHeight(_date, _nettype){
    ...
    var secondsPerBlockV1 = 120;
    // time of v2 fork 2016-03-23 15:57:38 (1458748658)
    var forkTime = _nettype == "Mainnet" ? 1627060976 : _nettype == "Testnet" ? 1627060976 : 1627060976;
    // v2 fork block
    var forkBlock = _nettype == "Mainnet" ? 1 : _nettype == "Testnet" ? 1 : 1;
    ...
    var secondsPerBlock;
    // before liberty's birth
    if (requestedTime < libertyBirthTime){
        console.log("Calculated blockchain height: 0, requestedTime < libertyBirthTime " );
        return 0;
    }
    // time between during v1
    if (requestedTime > libertyBirthTime && requestedTime < forkTime){
        approxBlockchainHeight = Math.floor((requestedTime - libertyBirthTime)/secondsPerBlockV1);
